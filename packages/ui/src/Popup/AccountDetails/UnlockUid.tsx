...
export const DecodeUid: FC<Props> = ({ decode, onClose }) => {
  ...
  return (
    <>
      <Flex justifyContent="space-between" m="m">
        <Box>
          <Text color="gray1" variant="c1">
            SHOW MY uID
          </Text>
        </Box>
        <Icon
      ...
      >
        <Box>
          <Box>
            <Text color="gray3" variant="b2">
              Your uID is a unique identifier attached to a single person or
              entity in the real world. A person or entity can have multiple
              Polymesh Accounts, but only one uID.
            </Text>
          </Box>
          <Box mt="l">
        ...
        <Box style={{ width: '100%' }}>
          <Flex mb="s" style={{ width: '100%' }}>
            <Flex flex={1}>
              <Button fluid onClick={onClose} variant="secondary">
                Cancel
              </Button>
            </Flex>
            <Flex flex={1} ml="8px">
              <Button fluid form="passwordForm" type="submit">
                Show my uID
              </Button>
            </Flex>
          </Flex>
