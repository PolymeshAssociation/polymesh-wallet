...
import { useLedger, useMetadata } from '@polymeshassociation/extension-ui/hooks';
...
import { Box, Button, Flex, Header, Icon, Loading, Text, TextInput } from '@polymeshassociation/extension-ui/ui';
...
function ImportLedger (): React.ReactElement {
  ...
  const isConnecting = status === Status.Loading || status === Status.Pending;
      ...
      {isConnecting
        ...
        : status !== Status.Ok
          ? (
            ...
            >
              <TroubleshootGuide
                error={ledgerData.error}
                headerText='Your Ledger is not connected'
                ledgerStatus={status}
                rawError={ledgerData.rawError}
                refresh={refresh}
              />
            </Box>
          )
