...
export async function subscribePolySelectedAccount(
  cb: (selected: string | undefined) => void
): Promise<boolean> {
  return polyMessage('poly:pri(selectedAccount.subscribe)', null, cb);
}

export async function setPolySelectedAccount(
  account: string
): Promise<boolean> {
...
export async function subscribePolyStatus(
...
export async function subscribeMetadataRequests(
  cb: (requests: MetadataRequest[]) => void
): Promise<boolean> {
  return sendMessage('pri(metadata.requests)', null, cb);
}

export async function subscribeSigningRequests(
  cb: (requests: SigningRequest[]) => void
): Promise<boolean> {
...
export async function changePassword(
  ...
  return sendMessage('pri(accounts.changePassword)', {
    address,
    newPass,
    oldPass,
  });
...
export async function globalChangePass(
  oldPass: string,
  newPass: string
): Promise<boolean> {
